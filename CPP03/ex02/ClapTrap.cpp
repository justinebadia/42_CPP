...
ClapTrap::ClapTrap()
{
    ...
    _energyPoint = 10;
    _attackDamage = 0;
    std::cout << VIO "Le constructeur par défaut créait le ClapTrap" << _name << NC << std::endl;
}
...
void    ClapTrap::takeDamage(unsigned int amount)
{
    _energyPoint -= amount;
	if (_energyPoint > 0)
    	std::cout << RED << _name << " se fait attaquer de " << amount << ". Il ne lui reste plus que " << _energyPoint << " de vie." NC << std::endl;
	else
